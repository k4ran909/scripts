...
cmd_install() {
    ...
    # Stop existing service before install/upgrade
...
# ── Create mtg config (verified format from official docs) ───
create_config() {
    info "Creating config..."

    cat > "$CONFIG_FILE" << EOF
# MTG v2 Configuration — auto-generated by telegram_proxy.sh
# Docs: https://github.com/9seconds/mtg

# Secret (FakeTLS, disguised as ${FAKETLS_DOMAIN})
secret = "${SECRET}"

# Listen address
bind-to = "0.0.0.0:${PROXY_PORT}"

# Max simultaneous connections (high for download-heavy usage)
concurrency = 16384

# Prefer IPv4 for direct Telegram DC connections (faster in Asia)
prefer-ip = "prefer-ipv4"

# Network timeouts tuned for high-throughput downloads
[network.timeout]
tcp = "5s"
http = "10s"
idle = "10m"
...
# TCP keepalive — aggressive to keep mobile connections alive
[network.keep-alive]
disabled = false
idle = "10s"
interval = "10s"
...
# ── TCP buffers (16 MB max for big Telegram downloads) ──
...
# ── BBR congestion control (massive improvement for long-distance) ──
...
# ── Enable window scaling & timestamps ──
...
SYSCTL
    ...
    ok "BBR + TCP buffers + Fast Open + window scaling enabled"
}

# ── Create systemd service (from official mtg docs) ──────────
create_service() {
...
cmd_doctor() {
    info "Running mtg doctor (connectivity check)..."
    "$MTG_BIN" doctor "$CONFIG_FILE"
}
...
cmd_help() {
    ...
    echo "║  doctor      Run connectivity diagnostics                ║"
